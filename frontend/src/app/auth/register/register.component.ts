    ...
    registerUser(): void {
        ...
        this.userService.register(this.user).subscribe((response) => {
            ...
            if (response.message === 'User already exist') {
                Swal.fire({icon: 'error', title: 'Registration Failed', text: 'User already exists. Please try with a different username.', confirmButtonText: 'OK'});
            } else {
                Swal.fire({icon: 'success', title: 'Registration Successful', text: 'You have successfully registered.', confirmButtonText: 'OK'}).then(() => {
                    this.router.navigateByUrl('/auth/login');
                });
            }
        }, (error) => {
            console.error('Registration failed:', error);
            Swal.fire({icon: 'error', title: 'Registration Failed', text: 'Failed to register. Please try again later.', confirmButtonText: 'OK'});
        });
    }
